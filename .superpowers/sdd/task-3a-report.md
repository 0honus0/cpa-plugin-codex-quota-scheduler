...
- Strict migration also recursively inspects every decoded allowed string value with the centralized project credential-syntax detector (Cookie/raw token assignment/Authorization/Bearer/secret-field patterns). Credential material embedded in aliases, notes, tags, groups, or config strings is rejected without creating `.user-data.json` or renaming the legacy file. Benign prose mentioning “token” remains migratable, and the actual retained `<legacy>.migrated` bytes are scanned with the same helper.

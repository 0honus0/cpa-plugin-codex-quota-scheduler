...
func RegisterManagement() pluginapi.ManagementRegistrationResponse {
	return pluginapi.ManagementRegistrationResponse{
		Resources: []pluginapi.ResourceRoute{
			...
			{
				Path:        "/status-data",
				Description: "Read cached scheduler quota status data.",
			},
		},
...
func HandleManagementRequest(store *PluginState, req pluginapi.ManagementRequest, now time.Time) pluginapi.ManagementResponse {
	...
	switch {
	case method == http.MethodGet && path == "/status-data" && isResourcePath(req.Path):
...
const PUBLIC_STATUS_BASE='/v0/resource/plugins/codex-quota-scheduler';
...
async function requestPublicStatus(){const resp=await fetch(PUBLIC_STATUS_BASE+'/status-data',{cache:'no-store'});const data=await readJSON(resp);if(!resp.ok)throw new Error(data.error||data.message||t('error.requestFailed',{status:resp.status}));return data}
...
function resetCreditSummary(account){const parts=[];if(account.reset_credits_available_count==null){parts.push('暂无')}else{parts.push(String(account.reset_credits_available_count)+' 次')}if(account.reset_credits_total_earned_count!=null)parts.push('/ 累计 '+account.reset_credits_total_earned_count+' 次');for(const credit of account.reset_credits||[]){const raw=credit.expires_at||'';if(!raw)continue;const date=new Date(raw);const expiry=Number.isNaN(date.getTime())?raw:date.toLocaleString('zh-CN',{hour12:false});parts.push('；'+(credit.status?credit.status+' ':'')+'有效期 '+expiry)}return parts.join(' ')}
...
async function refreshLogs(){try{await refreshStatus()}catch(error){showNotice(error.message||String(error),true)}}
localeSelect.addEventListener('change',()=>changeLocale(localeSelect.value));
document.getElementById('managementKey').addEventListener('change',()=>refreshStatus({fillSettings:true}).catch((error)=>showNotice(error.message||String(error),true)));
document.getElementById('managementKey').addEventListener('blur',()=>refreshStatus({fillSettings:true}).catch((error)=>showNotice(error.message||String(error),true)));
...
formatLocalTimes();
applyLocale();
refreshStatus({fillSettings:true}).catch((error)=>showNotice(error.message||String(error),true));

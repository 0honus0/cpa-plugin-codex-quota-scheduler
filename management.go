...
<div class="actions primary-actions"><button id="refreshQuota" type="button" class="secondary" data-i18n="actions.refreshQuota">Refresh Quota</button></div>
<details class="section collapsible" id="settingsPanel"><summary><span class="summary-text"><span class="summary-title" data-i18n="settings.title">Scheduler Settings</span><span class="summary-subtitle" data-i18n="settings.summary">Default configuration is ready; normally no manual changes are needed.</span></span></summary><div class="collapsible-body">
...
<label class="field"><span data-i18n="settings.refreshActiveWindow">Refresh active window</span><input id="refreshActiveWindow" spellcheck="false"></label>
<label class="field"><span data-i18n="settings.refreshAfterResetDelay">Refresh after reset delay</span><input id="refreshAfterResetDelay" spellcheck="false"></label>
<label class="field"><span data-i18n="settings.refreshRetryDelays">Refresh retry delays</span><input id="refreshRetryDelays" spellcheck="false"></label>
<label class="toggle"><span data-i18n="settings.refreshOnStartup">Refresh on startup</span><input id="refreshOnStartup" type="checkbox"></label>
...
<div class="actions"><button id="saveSettings" type="button" data-i18n="actions.saveSettings">Save Settings</button><button id="exportConfig" type="button" class="ghost" data-i18n="actions.exportConfig">Export Config</button><button id="importConfig" type="button" class="ghost" data-i18n="actions.importConfig">Import Config</button><input id="importFile" type="file" accept="application/json,.json" hidden></div>
</div></details>
...
async function refreshStatus(){const data=await requestManagement('/status',{query:{format:'json'}});STATUS=data;rebuildDerivedState();fillSettings();renderAccounts(STATUS.accounts||[]);renderLogs(STATUS.logs||[]);applyLocale()}
...
function addKV(parent,key,value){parent.append(node('span','',key),node('span','',value||'None'))}
function addBadge(text,className){return node('span','badge '+(className||''),text)}
function addQuota(windowData,label){const row=node('div','quota-row');const head=node('div','quota-head');head.append(node('span','quota-title',label||windowData.label||''),node('span','',windowData.display_text||''));if(!windowData.missing&&windowData.reset_text){const reset=node('span','quota-reset','');const time=node('span','localTime',windowData.reset_text);time.dataset.time=windowData.reset_text;reset.append(document.createTextNode((label||windowData.label||'Quota')+' reset: '),time);head.append(reset)}const bar=node('div','quota-bar');const fill=node('div','quota-fill '+(windowData.exhausted?'danger':((windowData.remaining_percent||0)<=20?'warn':'')));fill.style.width=Math.max(0,Math.min(100,Math.round(windowData.remaining_percent||0)))+'%';bar.append(fill);row.append(head,bar);return row}
function renderAccounts(accounts){const queue=document.querySelector('section.queue');if(!queue)return;queue.replaceChildren();const items=Array.isArray(accounts)?accounts:[];if(items.length===0){queue.append(node('div','empty','No account data yet.'));return}for(const account of items){const card=node('article','card '+(STATUS.next_auth_id&&STATUS.next_auth_id===account.auth_id?'next':''));card.dataset.authId=account.auth_id||'';const top=node('div','cardTop');const identity=node('div','identity');const titleLine=node('div','titleLine');titleLine.append(node('span','title',account.alias||account.auth_id||''));if(account.group)titleLine.append(node('span','groupPill',account.group));identity.append(titleLine);const sub=node('div','sub');const code=document.createElement('code');code.textContent=account.auth_id||'';sub.append(code);identity.append(sub);if(account.tags&&account.tags.length){const tags=node('div','metaLine');for(const tag of account.tags)tags.append(node('span','chip',tag));identity.append(tags)}top.append(identity,node('span','rank','#'+(account.rank||'')));card.append(top);const badges=node('div','badges');if(STATUS.next_auth_id&&STATUS.next_auth_id===account.auth_id)badges.append(addBadge('Next preferred','next'));badges.append(account.available?addBadge('Available','ok'):addBadge(account.unavailable_reason||'Unavailable','no'));badges.append(addBadge(account.family||'unknown type'),addBadge('CPA priority '+(account.cpa_priority||0)),addBadge('Circuit: '+((account.circuit&&account.circuit.label)||'')));if(account.auth_failure)badges.append(addBadge('Please re-login','no'));if(account.refresh_due_reason)badges.append(addBadge(account.refresh_due_reason));card.append(badges);const quotaList=node('div','quotaList');if(account.five_hour&&!account.five_hour.missing)quotaList.append(addQuota(account.five_hour,'5-hour quota'));if(account.long_window)quotaList.append(addQuota(account.long_window,account.long_window.label||'Long quota'));card.append(quotaList);const kv=node('div','kv');addKV(kv,'Cache age',account.cache_age||'None');addKV(kv,'Circuit count','Failures '+((account.circuit&&account.circuit.failure_count)||0)+' / Successes '+((account.circuit&&account.circuit.success_count)||0));addKV(kv,'Reset credits',account.reset_credits_available_count==null?'None':String(account.reset_credits_available_count));if(account.last_error)addKV(kv,'Last error',account.last_error);if(account.next_retry_text)addKV(kv,'Next retry',account.next_retry_text);card.append(kv);if(account.notes||account.group_notes){const notes=node('div','noteBlock');if(account.notes)notes.append(node('div','',account.notes));if(account.group_notes)notes.append(node('div','',account.group_notes));card.append(notes)}const actions=node('div','cardActions');const refresh=node('button','ghost refreshOne','Refresh Quota');refresh.type='button';refresh.addEventListener('click',()=>refreshOneQuota(account.auth_id||''));const edit=node('button','secondary openEdit','Edit');edit.type='button';edit.addEventListener('click',()=>openEdit(account.auth_id||''));actions.append(refresh,edit);card.append(actions);queue.append(card)}formatLocalTimes()}
...
async function importConfigFile(file){if(!file)return;try{const text=await file.text();await requestManagement('/import',{method:'POST',body:text});showNotice(t('notice.configImported'),false);await refreshStatus()}catch(error){showNotice(error.message||String(error),true)}}
